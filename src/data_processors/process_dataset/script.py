...
output_train = subset_h5ad_by_format(
    adata[[not x for x in is_test]], config, "output_train", slot_mapping
)

print(">> Creating test data", flush=True)
output_test = subset_h5ad_by_format(adata[is_test], config, "output_test", slot_mapping)

print(">> Creating solution data", flush=True)
output_solution = subset_h5ad_by_format(
    adata[is_test], config, "output_solution", slot_mapping
)
